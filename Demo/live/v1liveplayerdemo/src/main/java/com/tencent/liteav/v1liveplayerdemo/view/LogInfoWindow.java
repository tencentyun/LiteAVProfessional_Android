...
public class LogInfoWindow extends PopupWindow {
    ...
    private V1BalanceSeekBar mSeekBarBalance;
    ...
    private void initView() {
        View rootView = LayoutInflater.from(mContext).inflate(R.layout.v1liveplayer_view_loginfo, null);
        setContentView(rootView);
        mLayout = rootView.findViewById(R.id.v1liveplayer_ll_layout);
        mImageNetSpeed = rootView.findViewById(R.id.v1liveplayer_iv_net_speed);

        mBalanceView = LayoutInflater.from(mContext).inflate(R.layout.v1liveplayer_view_loginfo_balance, null);
        mProgressVideoCache = mBalanceView.findViewById(R.id.v1liveplayer_pb_video_cache);
        mProgressAudioCache = mBalanceView.findViewById(R.id.v1liveplayer_pb_audio_cache);
        mSeekBarBalance = mBalanceView.findViewById(R.id.v1player_seekbar_balance);
        mSeekBarBalance.setEnabled(false);
        mTextMaxAutoAdjustCacheTime = mBalanceView.findViewById(R.id.v1liveplayer_tv_max_auto_adjust_cache_time);
    }

    private void initData() {
        List<LogInfo> list = new ArrayList<>();
        list.add(LogInfo.createModelLogInfo(TUIBuild.getModel()));
        list.add(LogInfo.createVersionLogInfo(TUIBuild.getReleaseVersion()));
    ...
    private void handleLogStatus(Bundle status) {
        ...
        if (mNetSpeed > 1500) {
            mImageNetSpeed.setImageResource(R.drawable.v1liveplayer_ic_net_speed5);
        } else if (mNetSpeed > 1000) {
            mImageNetSpeed.setImageResource(R.drawable.v1liveplayer_ic_net_speed4);
        } else if (mNetSpeed > 500) {
            mImageNetSpeed.setImageResource(R.drawable.v1liveplayer_ic_net_speed3);
        } else if (mNetSpeed > 250 && speed <= 500) {
            mImageNetSpeed.setImageResource(R.drawable.v1liveplayer_ic_net_speed2);
        } else if (mNetSpeed > 0 && mNetSpeed <= 250) {
            mImageNetSpeed.setImageResource(R.drawable.v1liveplayer_ic_net_speed1);
        } else if (mNetSpeed == 0) {
            mImageNetSpeed.setImageResource(R.drawable.v1liveplayer_ic_net_speed0);
        }
        ...
        if (mMaxAutoAdjustCacheTime != 0) {
            ...
            float videoCacheTs = status.getInt(TXLiveConstants.NET_STATUS_VIDEO_CACHE) * 1.0f / 1000;//s
            float audioCacheTs = status.getInt(TXLiveConstants.NET_STATUS_AUDIO_CACHE) * 1.0f / 1000;//s

            int vp = (int) (videoCacheTs * 100 / mMaxAutoAdjustCacheTime);
            int ap = (int) (audioCacheTs * 100 / mMaxAutoAdjustCacheTime);
            mProgressVideoCache.setProgress(vp);
            mProgressAudioCache.setProgress(ap);
            Log.i(TAG,
                    "max:" + mMaxAutoAdjustCacheTime + " videoCacheTs:" + videoCacheTs + " audioCacheTs:" + audioCacheTs
                            + ",vp:" + vp + ",ap:" + ap);
        }
            ...
            switch (eventId) {
                ...
                case TXLiveConstants.PLAY_EVT_START_VIDEO_DECODER:
                    ...
                    if (mode == DECODER_HARD) {
                        updateItemText(LogInfo.createStep2LogInfo(
                                getResources().getString(R.string.v1liveplayer_text_hw_decode_start)));
                    } else {
                        updateItemText(LogInfo.createStep2LogInfo(
                                getResources().getString(R.string.v1liveplayer_text_sw_decode_start)));
                    }
                ...
                case TXLiveConstants.PLAY_EVT_RCV_FIRST_I_FRAME:
                    ...
                    break;
                default:
                    Log.e(TAG, "handleLogEvent wrong event id : " + eventId);
                    break;
            }
        }
    }
    ...
    private class ItemView extends LinearLayout {
        ...
        private void init() {
            ...
            mTextView.setTextColor(getResources().getColor(R.color.v1liveplayer_white));
        ...
        public void setStatus(int status) {
            switch (status) {
                ...
                case LogInfo.STATUS_FAIL:
                    ...
                    mImageView.setBackgroundResource(R.drawable.v1liveplayer_log_info_step_fail);
                    break;
                case LogInfo.STATUS_SUCCESS:
                    mImageView.setVisibility(View.VISIBLE);
                    mImageView.setBackgroundResource(R.drawable.v1liveplayer_log_info_step_success);
                    break;
                default :
                    Log.e(TAG, "setStatus wrong status : " + status);
                    break;
            }
        }
    ...
    public static class LogInfo {
        ...
        public static LogInfo createModelLogInfo(String value) {
            return new LogInfo(ID_MODEL, getResources().getString(R.string.v1liveplayer_model), value);
        }

        public static LogInfo createVersionLogInfo(String value) {
            return new LogInfo(ID_VERSION, getResources().getString(R.string.v1liveplayer_version), value);
        }

        public static LogInfo createDownloadSpeedLogInfo(String value) {
            return new LogInfo(ID_DOWNLOAD_SPEED, getResources().getString(R.string.v1liveplayer_text_download_speed),
                    value);
        }

        public static LogInfo createResolutionLogInfo(String value) {
            return new LogInfo(ID_RESOLUTION, getResources().getString(R.string.v1liveplayer_text_resolution_detail),
                    value);
        }

        public static LogInfo createFpsGopLogInfo(String value) {
            return new LogInfo(ID_FPS_GOP, getResources().getString(R.string.v1liveplayer_text_fps), value);
        }

        public static LogInfo createStep1LogInfo(String value) {
            return new LogInfo(ID_STEP_1, STATUS_FAIL, getResources().getString(R.string.v1liveplayer_start_step1),
                    value);
        }

        public static LogInfo createStep1LogInfo(int status) {
            return new LogInfo(ID_STEP_1, status, getResources().getString(R.string.v1liveplayer_start_step1), "");
        }

        public static LogInfo createStep2LogInfo(String value) {
            return new LogInfo(ID_STEP_2, STATUS_FAIL, getResources().getString(R.string.v1liveplayer_start_step2),
                    value);
        }

        public static LogInfo createStep2LogInfo(int status) {
            return new LogInfo(ID_STEP_2, status, getResources().getString(R.string.v1liveplayer_start_step2), "");
        }

        public static LogInfo createStep3LogInfo(String value) {
            return new LogInfo(ID_STEP_3, STATUS_FAIL, getResources().getString(R.string.v1liveplayer_start_step3),
                    value);
        }

        public static LogInfo createStep3LogInfo(int status) {
            return new LogInfo(ID_STEP_3, status, getResources().getString(R.string.v1liveplayer_start_step3), "");
        }

        public static LogInfo createStep4LogInfo(String value) {
            return new LogInfo(ID_STEP_4, STATUS_FAIL, getResources().getString(R.string.v1liveplayer_start_step4),
                    value);
        }

        public static LogInfo createStep4LogInfo(int status) {
            return new LogInfo(ID_STEP_4, status, getResources().getString(R.string.v1liveplayer_start_step4), "");
        }

        public static LogInfo createStep5LogInfo(String value) {
            return new LogInfo(ID_STEP_5, STATUS_FAIL, getResources().getString(R.string.v1liveplayer_start_step5),
                    value);
        }

        public static LogInfo createStep5LogInfo(int status) {
            return new LogInfo(ID_STEP_5, status, getResources().getString(R.string.v1liveplayer_start_step5), "");
        }
