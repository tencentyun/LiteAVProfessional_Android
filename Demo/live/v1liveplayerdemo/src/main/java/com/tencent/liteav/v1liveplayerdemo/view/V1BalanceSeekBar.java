 ...
 * 平衡点 View
 */
public class V1BalanceSeekBar extends AppCompatSeekBar {
    private Paint  mPaint;            //定义此控件的画笔对象
    private String mTitleText = "";   //保存此SeekBar的文本信息

    public V1BalanceSeekBar(Context context) {
        this(context, null);
    }

    public V1BalanceSeekBar(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public V1BalanceSeekBar(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray typedArray = context.getTheme().obtainStyledAttributes(attrs,
                R.styleable.V1LivePlayerTCSeekBarWithText, defStyleAttr, 0);

        for (int i = 0; i < typedArray.getIndexCount(); i++) {
            int attr = typedArray.getIndex(i);
            if (attr == R.styleable.V1LivePlayerTCSeekBarWithText_textSize) {
                mPaint.setTextSize(typedArray.getDimension(attr, 14f));
            } else if (attr == R.styleable.V1LivePlayerTCSeekBarWithText_textColor) {
