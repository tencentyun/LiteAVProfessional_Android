...
import com.tencent.liteav.v1livepusherdemo.fragment.V1VisibleLogListAdapter;
...
public class TXPushVisibleLogView extends RelativeLayout implements View.OnClickListener {
    ...
    public TXPushVisibleLogView(Context context, AttributeSet attrs) {
        ...
        LayoutInflater.from(context).inflate(R.layout.v1livepusher_push_visible_log_view, this);

        mCatonRecyclerView = (RecyclerView) findViewById(R.id.v1livepusher_recycler_view);
        mCatonRecyclerView.setLayoutManager(new LinearLayoutManager(context));
        mAdapter = new V1VisibleLogListAdapter(context);
        mAdapter.setType(V1VisibleLogListAdapter.TYPE_DROP);
        mCatonRecyclerView.setAdapter(mAdapter);

        mTvFpsGop = (TextView) findViewById(R.id.v1livepusher_tv_fpsgop);
        mTvBrand = (TextView) findViewById(R.id.v1livepusher_tv_brand);
        mTvBitrate = (TextView) findViewById(R.id.v1livepusher_tv_bitrate);
        mTvSpeed = (TextView) findViewById(R.id.v1livepusher_tv_speed);
        mTvEncoder = (TextView) findViewById(R.id.v1livepusher_tv_seg4);
        mIvClose = (ImageView) findViewById(R.id.v1livepusher_iv_close);
        mIvQue = (ImageView) findViewById(R.id.v1livepusher_iv_que);
        mIvStep1 = (ImageView) findViewById(R.id.v1livepusher_iv_step1);
        mIvStep2 = (ImageView) findViewById(R.id.v1livepusher_iv_step2);
        mIvStep3 = (ImageView) findViewById(R.id.v1livepusher_iv_step3);
        mIvStep4 = (ImageView) findViewById(R.id.v1livepusher_iv_step4);
        mIvStep5 = (ImageView) findViewById(R.id.v1livepusher_iv_step5);

        mIvClose.setOnClickListener(this);
        mTvBrand.setText(String.format(
                context.getString(R.string.v1livepusher_type_and_android_version),
                TUIBuild.getModel(),
                ...
                switch (eventId) {
                    ...
                    case PUSH_EVT_START_VIDEO_ENCODER: //编码器启动 step4
                        ...
                        if (mode == ENCODER_HARD) { // 硬编码器1 软编码器2
                            mTvEncoder.setText(String.format("阶段四：编码器正常启动[%s]", "264硬编"));
                        } else if (mode == ENCODER_SOFT) {
                            mTvEncoder.setText(String.format("阶段四：编码器正常启动[%s]", "264软编"));
                        } else if (mode == ENCODER_HARD_265) {
                            mTvEncoder.setText(String.format("阶段四：编码器正常启动[%s]", "h265硬编"));
                        } else {
                            mTvEncoder.setText(String.format("阶段四：编码器正常启动[%s]", "h265软编"));
                        }
                        break;
