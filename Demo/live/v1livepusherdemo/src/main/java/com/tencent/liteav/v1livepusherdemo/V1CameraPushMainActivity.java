...
import com.tencent.liteav.v1livepusherdemo.fragment.V1PusherPlayQRCodeFragment;
import com.tencent.liteav.v1livepusherdemo.fragment.V1PusherSettingFragment;
import com.tencent.liteav.v1livepusherdemo.fragment.V1PusherVideoQualityFragment;
...
import com.tencent.liteav.v1livepusherdemo.view.LogInfoWindow;
 ...
 */
public class V1CameraPushMainActivity extends FragmentActivity implements ITXLivePushListener,
        V1PusherVideoQualityFragment.OnVideoQualityChangeListener, V1PusherSettingFragment.OnSettingChangeListener {
    private static final String TAG                           = "V1CameraPushMain";
    ...
    private V1PusherPlayQRCodeFragment   mFragmentPusherPlayQRCode;   // 拉流地址面板
    private V1PusherSettingFragment      mFragmentPusherSetting;      // 设置面板
    private V1PusherVideoQualityFragment mFragmentPusherVideoQuality; // 画质面板
    ...
    private String      mAacPlayURL              = ""; // 低延时拉流地址
    private boolean     mIsEnableHighCapture     = false;
    private boolean     mIsCustomVideoPreProcess = false;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.v1livepusher_main_activity);
        initData();                // 初始化数据
        ...
        startPush();
        mFragmentPusherPlayQRCode.toggle(getFragmentManager(), PUSHER_PLAY_QR_CODE_FRAGMENT);
    }
    ...
    private void handleClickShowLog(View view) {
    ...
    public void onClick(View view) {
        int id = view.getId();
        if (id == R.id.v1livepusher_ibtn_back) {
            stopPush();
            finish();
        } else if (id == R.id.v1livepusher_ibtn_show_log) {
            handleClickShowLog(view);
        } else if (id == R.id.v1livepusher_ibtn_qrcode) {
            if (mLogInfoWindow.isShowing()) {
                mLogInfoWindow.dismiss();
            }
            mFragmentPusherPlayQRCode.toggle(getFragmentManager(), PUSHER_PLAY_QR_CODE_FRAGMENT);
        } else if (id == R.id.v1livepusher_btn_start) {
            togglePush();
        } else if (id == R.id.v1livepusher_btn_switch_camera) {
            // 表明当前是前摄像头
            if (view.getTag() == null || (Boolean) view.getTag()) {
                view.setTag(false);
                view.setBackgroundResource(R.drawable.v1livepusher_camera_back_btn);
            } else {
                view.setTag(true);
                view.setBackgroundResource(R.drawable.v1livepusher_camera_front);
            }
            switchCamera();
        } else if (id == R.id.v1livepusher_btn_beauty) {
            if (mLogInfoWindow.isShowing()) {
                mLogInfoWindow.dismiss();
            }
        ...
        } else if (id == R.id.v1livepusher_btn_bgm) {
            if (mLogInfoWindow.isShowing()) {
                mLogInfoWindow.dismiss();
            }
        ...
        } else if (id == R.id.v1livepusher_btn_video_quality) {
            if (mLogInfoWindow.isShowing()) {
                mLogInfoWindow.dismiss();
            }
            mFragmentPusherVideoQuality.toggle(getSupportFragmentManager(), PUSHER_VIDEO_QUALITY_FRAGMENT);
        } else if (id == R.id.v1livepusher_btn_setting) {
            if (mLogInfoWindow.isShowing()) {
                mLogInfoWindow.dismiss();
            }
            mFragmentPusherSetting.toggle(getSupportFragmentManager(), PUSHER_SETTING_FRAGMENT);
        }
    }
    ...
    public void onPushEvent(int event, Bundle param) {
        ...
        String pushEventLog = getString(R.string.v1livepusher_receive_event) + event + ", " + msg;
    ...
    public void onTouchFocusChange(boolean enable) {
        ...
        if (mIsPushing) {
            showToast(R.string.v1livepusher_pushing_start_stop_retry_push_by_focus);
        }
    }
    ...
    public void onEnableZoomChange(boolean enable) {
        ...
        if (mIsPushing) {
            showToast(R.string.v1livepusher_pushing_start_stop_retry_push);
        }
    }
    ...
    public void onAdjustBitrateChange(boolean enable) {
        setAdjustBitrate(enable, mFragmentPusherVideoQuality.getQualityType());
    }

    @Override
    public void onQualityChange(int type) {
        setQuality(mFragmentPusherSetting.isAdjustBitrate(), type);
    }
    ...
    private void initData() {
        ...
        mAacPlayURL = intent.getStringExtra(Constants.INTENT_URL_PLAY_ACC);

        mProfileMode = intent.getIntExtra(V1Constants.KEY_PROFILE_MODE, 0);
        mIsCustomVideoPreProcess = intent.getBooleanExtra(V1Constants.KEY_CUSTOM_VIDEO_PREPROCESS, false);

        mProfileMode = intent.getIntExtra(V1Constants.KEY_PROFILE_MODE, 0);
        mIsEnableHighCapture = intent.getBooleanExtra(V1Constants.KEY_ENABLE_HIGH_CAPTURE, false);

        mPauseFlag = TXLiveConstants.PAUSE_FLAG_PAUSE_VIDEO;
        if (intent.getBooleanExtra(V1Constants.KEY_PAUSE_AUDIO_ON_ACTIVITY_PAUSED, false)) {
            mPauseFlag |= TXLiveConstants.PAUSE_FLAG_PAUSE_AUDIO;
        }
    }
    ...
    private void initPusher() {
        mPusherView = findViewById(R.id.v1livepusher_tx_cloud_view);
        ...
        setMute(mFragmentPusherSetting.isMute());
        setMirror(mFragmentPusherSetting.isMirror());
        setWatermark(mFragmentPusherSetting.isWatermark());
        setTouchFocus(mFragmentPusherSetting.isTouchFocus());
        setEnableZoom(mFragmentPusherSetting.isEnableZoom());
        enablePureAudioPush(mFragmentPusherSetting.enablePureAudioPush());
        enableAudioEarMonitoring(mFragmentPusherSetting.enableAudioEarMonitoring());
        setQuality(mFragmentPusherSetting.isAdjustBitrate(), mFragmentPusherVideoQuality.getQualityType());
        setAudioQuality(mFragmentPusherSetting.getAudioChannels(), mFragmentPusherSetting.getAudioSampleRate());
        setHomeOrientation(mFragmentPusherSetting.isLandscape());
        turnOnFlashLight(mFragmentPusherSetting.isFlashEnable());
        setHardwareAcceleration(mFragmentPusherSetting.isHardwareAcceleration());
    }
    ...
    private void initFragment() {
        if (mFragmentPusherSetting == null) {
            mFragmentPusherSetting = new V1PusherSettingFragment();
            mFragmentPusherSetting.loadConfig(this);
            mFragmentPusherSetting.setOnSettingChangeListener(this);
        }
        if (mFragmentPusherPlayQRCode == null) {
            mFragmentPusherPlayQRCode = new V1PusherPlayQRCodeFragment();
            mFragmentPusherPlayQRCode.setQRCodeURL(mFlvPlayURL, mRTMPPlayURL, mHlsPlayURL, mAacPlayURL);
        }
        if (mFragmentPusherVideoQuality == null) {
            mFragmentPusherVideoQuality = new V1PusherVideoQualityFragment();
            mFragmentPusherVideoQuality.loadConfig(this);
            mFragmentPusherVideoQuality.setOnVideoQualityChangeListener(this);
        }
    ...
    private void initMainView() {
        mBtnStartPush = findViewById(R.id.v1livepusher_btn_start);
        mBeautyPanelView = findViewById(R.id.v1livepusher_bp_beauty_pannel);
        mTextNetBusyTips = findViewById(R.id.v1livepusher_tv_net_error_warning);
        mLinearBottomBar = findViewById(R.id.v1livepusher_ll_bottom_bar);

        mAudioEffectPanel = findViewById(R.id.v1livepusher_audio_panel);
    ...
    private void showToast(final String text) {
        if (Looper.myLooper() == Looper.getMainLooper()) {
            ...
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    Toast.makeText(V1CameraPushMainActivity.this, text, Toast.LENGTH_SHORT).show();
                }
            });
        }
    }
    ...
    private void onPushStart(int code) {
        ...
        switch (code) {
            case Constants.PLAY_STATUS_SUCCESS:
                mBtnStartPush.setBackgroundResource(R.drawable.v1livepusher_pause);
                break;
            case Constants.PLAY_STATUS_INVALID_URL:
                showToast(R.string.v1livepusher_url_illegal);
                // 输出状态log
                Bundle params = new Bundle();
                params.putString(TXLiveConstants.EVT_DESCRIPTION, getString(R.string.v1livepusher_check_url));
                mLogInfoWindow.setLogText(null, params, LogInfoWindow.CHECK_RTMP_URL_FAIL);
                break;
            case Constants.PLAY_STATUS_LICENSE_ERROR:
                String errInfo = getString(R.string.v1livepusher_license_check_fail);
                int start = (errInfo + getString(R.string.v1livepusher_license_click_info)).length();
                int end = (errInfo + getString(R.string.v1livepusher_license_click_use_info)).length();
                SpannableStringBuilder spannableStrBuidler =
                        new SpannableStringBuilder(errInfo + getString(R.string.v1livepusher_license_click_use_info));
                ClickableSpan clickableSpan = new ClickableSpan() {
                    ...
                    public void onClick(View view) {
                        ...
                        IntentUtils.safeStartActivity(V1CameraPushMainActivity.this, intent);
                    }
                };
                ...
                dialogBuilder.setTitle(getString(R.string.v1livepusher_push_fail)).setView(tv)
                        .setPositiveButton(getString(R.string.v1livepusher_comfirm),
        ...
        if (code != Constants.PLAY_STATUS_INVALID_URL) {
            ...
            bundle.putString(TXLiveConstants.EVT_DESCRIPTION, getString(R.string.v1livepusher_check_url));
    ...
    private void startPush() {
        ...
        if (TextUtils.isEmpty(tRTMPURL) || (!tRTMPURL.trim().toLowerCase().startsWith("rtmp://"))) {
            ...
            mLivePushConfig.enableHighResolutionCaptureMode(mIsEnableHighCapture);
            mLivePushConfig.setPauseFlag(mPauseFlag);
    ...
    private void stopPush() {
        ...
        mIsPushing = false;
        if (mFragmentPusherSetting != null) {
            mFragmentPusherSetting.closePrivateModel();
        }
        mBtnStartPush.setBackgroundResource(R.drawable.v1livepusher_start);
            ...
            public void onSnapshot(Bitmap bitmap) {
                if (mLivePusher.isPushing()) {
                    if (bitmap != null) {
                        ...
                        showToast(R.string.v1livepusher_screenshot_fail);
                    }
                } else {
                    showToast(R.string.v1livepusher_screenshot_fail_push);
                }
            }
        });
    }
    ...
    public void onFPSChange(int fps) {
        ...
        mLivePushConfig.setVideoFPS(fps);
        if (mLivePusher != null) {
            mLivePusher.setConfig(mLivePushConfig);
        }
    }
    ...
    private void setPushScene(int type, boolean enableAdjustBitrate) {
        ...
        // 是否开启硬件加速
        setHardwareAcceleration(mIsHWAcc);
    }
        ...
        AsyncTask.execute(new Runnable() {
            ...
            public void run() {
                ...
                if (file.exists() && file.length() > 0) {
                    showToast(R.string.v1livepusher_screenshot_success);
                    Intent intent = new Intent();
                    ...
                    IntentUtils.safeStartActivity(V1CameraPushMainActivity.this,
                            Intent.createChooser(intent, getString(R.string.v1livepusher_share_pic)));
                } else {
                    showToast(R.string.v1livepusher_screenshot_fail);
                }
            }
    ...
    private class ActivityRotationObserver extends ContentObserver {
        ...
        public void onChange(boolean selfChange) {
            ...
            if (isActivityCanRotation()) {
                mFragmentPusherSetting.hideOrientationItem();
            } else {
                mFragmentPusherSetting.showOrientationItem();
            }
        }
