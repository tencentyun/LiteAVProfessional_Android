...
public class TCCompressActivity extends FragmentActivity {
    ...
    private RadioButton                           mRadioVideoResolution1080p;
    ...
    private void initListener() {
        mTXVideoGenerateListener = new TXVideoEditer.TXVideoGenerateListener() {
            ...
            public void onGenerateProgress(final float progress) {
                Log.i(TAG, "onGenerateProgress = " + progress);
            ...
            public void onGenerateComplete(final TXVideoEditConstants.TXGenerateResult result) {
                Log.i(TAG, "onGenerateComplete result retCode = " + result.retCode);
                mCompressing = false;
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
    ...
    private void initView() {
        ...
        mRadioVideoResolution1080p = (RadioButton) findViewById(R.id.rb_video_compress_resolution_1080p);
        ...
        mRadioGroupVideoResolution.setOnCheckedChangeListener(new RadioGroup.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(RadioGroup group, int checkedId) {
                ...
                } else if (checkedId == mRadioVideoResolution1080p.getId()) {
                    mVideoResolution = TXVideoEditConstants.VIDEO_COMPRESSED_1080P;
                    mEtCompressBitrate.setText("12000");
                }
            }
        });
    ...
    private void stopCompressVideo() {
        if (!mCompressing) {
            Log.e(TAG, "stopCompressVideo, mCompressing is false, ignore");
            return;
        }
        if (mFragmentWorkLoadingProgress != null) {
            mFragmentWorkLoadingProgress.setProgress(0);
