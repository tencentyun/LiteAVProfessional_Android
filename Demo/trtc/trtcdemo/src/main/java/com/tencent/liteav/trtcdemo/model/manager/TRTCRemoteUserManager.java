...
public class TRTCRemoteUserManager {
    ...
    public TRTCRemoteUserManager(Context context,
        ...
        this(context, null, iView, isCustomRender, customRenderPixelFormat, customRenderBufferType, enableCustomProcess);
    }

    public TRTCRemoteUserManager(TRTCCloud trtcCloud,
                                 IView iView,
                                 boolean isCustomRender,
                                 int customRenderPixelFormat,
                                 int customRenderBufferType,
                                 boolean enableCustomProcess) {
        this(null, trtcCloud, iView, isCustomRender, customRenderPixelFormat, customRenderBufferType, enableCustomProcess);
    }

    private TRTCRemoteUserManager(Context context,
                                  TRTCCloud trtcCloud,
                                  IView iView,
                                  boolean isCustomRender,
                                  int customRenderPixelFormat,
                                  int customRenderBufferType,
                                  boolean enableCustomProcess) {
        mTRTCCloud = trtcCloud == null ? TRTCCloud.sharedInstance(context) : trtcCloud;
        mIView = iView;
    ...
    /**
     * 并发播放策略中，设置某一个用户的声音必定播放
     *
     * @param userId
     * @param isAdd
     */
    public Boolean setRemoteAudioParallelParams(String userId, Boolean isAdd) {
        if (!SettingConfigHelper.getInstance().getAudioConfig().setAudioParallelIncludeUsers(userId, isAdd)) {
            ToastUtils.showLong(R.string.trtcdemo_audio_parallel_must_play_fail_tips);
            return false;
        }

        TRTCCloudDef.TRTCAudioParallelParams params = new TRTCCloudDef.TRTCAudioParallelParams();
        params.maxCount = SettingConfigHelper.getInstance().getAudioConfig().getAudioParallelMaxCount();
        params.includeUsers = SettingConfigHelper.getInstance().getAudioConfig().getAudioParallelIncludeUsers();
    ...
    private void removeVideoStream(String userId, int streamType) {
        ...
        while (streamIterator.hasNext()) {
            ...
            if (stream.userId != null && stream.userId.equals(userId) && stream.streamType == streamType) {
                ...
                Log.i(TAG, "removeVideoStream " + userId + ", stream " + streamType + ", size " + mTRTCVideoStreams.size());
                break;
            }
        }
    }

    public void addRemoteUserToList(String userId) {
        RemoteUserConfig remoteUserConfig = RemoteUserConfigHelper.getInstance().getRemoteUser(userId, TRTCCloudDef.TRTC_VIDEO_STREAM_TYPE_BIG);
        if (remoteUserConfig == null) {
            remoteUserConfig = new RemoteUserConfig(userId, TRTCCloudDef.TRTC_VIDEO_STREAM_TYPE_BIG, mIsCustomRender);
        }
        RemoteUserConfigHelper.getInstance().addRemoteUser(remoteUserConfig);
    }

    public void removeRemoteUserFromList(String userId) {
        RemoteUserConfigHelper.getInstance().removeRemoteUser(userId, TRTCCloudDef.TRTC_VIDEO_STREAM_TYPE_BIG);
    }
    ...
    public void remoteUserVideoAvailable(String userId, int streamType, TXCloudVideoView renderView) {
        ...
        if (remoteUserConfig == null) {
            ...
            if (streamType == TRTCCloudDef.TRTC_VIDEO_STREAM_TYPE_SUB) {
                remoteUserConfig.setFillMode(remoteUserConfig.isFillMode());
                remoteUserConfig.setRotation(remoteUserConfig.getRotation());
                remoteUserConfig.setVolume(remoteUserConfig.getVolume());
                remoteUserConfig.setMirrorType(remoteUserConfig.getMirrorType());
            }
        }
        ...
        if (!isContainVideoStream(userId, TRTCCloudDef.TRTC_VIDEO_STREAM_TYPE_BIG)) {
            ...
            Log.i(TAG, "remoteUserVideoAvailable " + stream.userId +
    ...
    private void startSDKRender(String userId, int streamType, TXCloudVideoView renderView) {
        ...
        if (renderView != null) {
            ...
            TRTCCloudDef.TRTCRenderParams params = new TRTCCloudDef.TRTCRenderParams();
            if(remoteUserConfig != null){
                params.fillMode = remoteUserConfig.isFillMode() ? TRTCCloudDef.TRTC_VIDEO_RENDER_MODE_FILL : TRTCCloudDef.TRTC_VIDEO_RENDER_MODE_FIT;
                params.mirrorType = remoteUserConfig.getMirrorType();
                params.rotation = remoteUserConfig.getRotation();
            }
            mTRTCCloud.setRemoteRenderParams(userId, streamType, params);
    ...
    public void stopCloudMixture() {
        SettingConfigHelper.getInstance().getVideoConfig().setMixStreamId("");
    ...
    private void updateCloudMixture(int mode, ArrayList<TRTCVideoStream> localList, ArrayList<TRTCVideoStream> remoteList) {
        ...
        // 设置混流后各个小画面的位置
        int index = 0;
        Log.i(TAG, "updateCloudMixtureParams " + remoteList.size());
        for (TRTCVideoStream userStream : remoteList) {
            ...
            Log.i(TAG, "updateCloudMixtureParams userId " + _mixUser.userId);
