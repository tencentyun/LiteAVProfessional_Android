...
public class SubStreamBitrateItem extends AbsSeekBarItem {
    ...
    public SubStreamBitrateItem(int index, TRTCCloudManager manager, ArrayList<VideoSettingFragment.TRTCSettingBitrateTable> array, Context context, String title, String tip) {
        super(context, true, title, tip);
        mCurrentSolutionIndex = index;
        mTRTCCloudManager = manager;
        mParamArray = array;
        updateSolution(mCurrentSolutionIndex);
        setProgress(getBitrateProgress(SettingConfigHelper.getInstance().getVideoConfig().getSubStreamVideoBitrate(), mCurrentSolutionIndex));
        setTip(getBitrate(SettingConfigHelper.getInstance().getVideoConfig().getSubStreamVideoBitrate(), mCurrentSolutionIndex) + "kbps");
    ...
    public void onSeekBarChange(int index, boolean fromUser) {
        ...
        setTip(bitrate + "kbps");
        if (bitrate != SettingConfigHelper.getInstance().getVideoConfig().getSubStreamVideoBitrate()) {
            SettingConfigHelper.getInstance().getVideoConfig().setSubStreamVideoBitrate(bitrate);
